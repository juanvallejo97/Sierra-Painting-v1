...
if (!RUN_RULES) {
  test('Firestore rules tests skipped (FIRESTORE_EMULATOR_HOST not set)', () => {
    expect(true).toBe(true);
  });
} else {
  // Resolve firestore.rules path from project root robustly
  ...
  if (!rulesPath) {
    throw new Error('firestore.rules not found in expected locations');
  }
