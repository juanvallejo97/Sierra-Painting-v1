...
import { checkIdempotency, recordIdempotency, generateIdempotencyKey, isValidIdempotencyKey, } from '../lib/idempotency';
...
// MAIN FUNCTION
// ============================================================
export const markPaidManual = withValidation(ManualPaymentSchema, adminEndpoint({ /* functionName: 'markPaidManual' */}))(async (validatedPayment, req) => {
    // IDEMPOTENCY CHECK
    // ========================================
    ...
    if (validatedPayment.idempotencyKey && !isValidIdempotencyKey(idempotencyKey)) {
        throw new functions.https.HttpsError('invalid-argument', 'Invalid idempotency key format');
    }
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!invoiceDoc.exists) {
            throw new functions.https.HttpsError('not-found', `Invoice ${validatedPayment.invoiceId} not found`);
        }
        invoiceData = invoiceDoc.data();
        // Check if already paid
        if (invoiceData.paid) {
            throw new functions.https.HttpsError('failed-precondition', 'Invoice is already marked as paid');
        }
        ...
        if (validatedPayment.amount !== invoiceTotal) {
            ...
            throw new functions.https.HttpsError('invalid-argument', `Payment amount (${validatedPayment.amount}) does not match invoice total (${invoiceTotal})`);
        }
        // Update invoice
